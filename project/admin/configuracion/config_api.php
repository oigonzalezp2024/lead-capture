...
if ($action === 'list_comps') {
    $stmt = $pdo->query("SELECT * FROM question_options WHERE visible = 1");
...
if ($action === 'save_comp' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO question_options (id_option, id_question, option_label, option_value, next_route_map, visible) 
...
if ($action === 'list_ques') {
    $stmt = $pdo->prepare("SELECT * FROM survey_questions WHERE route = ? AND visible = 1 ORDER BY orden ASC");
...
if ($action === 'save_ques' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $d = json_decode(file_get_contents('php://input'), true);
    $stmt = $pdo->prepare("INSERT INTO survey_questions (codigo_pregunta, route, question_text, question_type, orden, visible) 
...
if ($action === 'delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $d = json_decode(file_get_contents('php://input'), true);
    $table = $d['type'] === 'comp' ? 'question_options' : 'survey_questions';
